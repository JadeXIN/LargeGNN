from load.kg import KG
from load.read import *


class KGs:
    def __init__(self, kg1: KG, kg2: KG, train_links, test_links, valid_links=None, mode='mapping', ordered=True, attr=False):
        if mode == "sharing":
...
def read_kgs_from_folder(training_data_folder, division, mode, ordered, remove_unlinked=False, attr=False, large=False):
    ...
    if large:
        links = read_links(training_data_folder + 'ent_links')
        print('ent_links length: ' + str(len(links)))
        # x = {i[0]:i[1] for i in links}
        # xx = {v:k for k, v in x.items()}
        # xxx = [(v, k) for k, v in xx.items()]
        # clean_link = []
        # for i in xxx:
        #     if ((kg1.hr_dict.get(i[0], -1) != -1 or kg1.rt_dict.get(i[0], -1) != -1)) and ((kg2.hr_dict.get(i[1], -1) != -1 or kg2.rt_dict.get(i[1], -1) != -1)):
        #         clean_link.append(i)
        # np.random.shuffle(clean_link)
        # print('cleaned link length: ' + str(len(clean_link)))
        train_links, valid_links = links[0:int(0.2 * len(links))], links[int(0.2 * len(links)):int(
            0.3 * len(links))]
        test_links = links[int(0.3 * len(links)):]
    else:
