...
def load_data_hard(lang,train_ratio = 0.3):
    entity1,rel1,triples1 = load_triples_hard(lang + 'triples_1.txt')
    entity2,rel2,triples2 = load_triples_hard(lang + 'triples_2.txt')
    entity2 = {i + len(entity1) for i in entity2}
    rel2 = {i + len(rel1) for i in rel2}
    triples2 = [(i[0] + len(entity1), i[1] + len(rel1), i[2] + len(entity1)) for i in triples2]
    alignment_pair = load_alignment_pair(lang + 'ref_ent_ids.txt')
    alignment_pair = [(i[0], i[1] + len(entity1)) for i in alignment_pair]
    train_pair = load_alignment_pair(lang + 'hard_split/train_entity_seeds.txt')
    train_pair = [(i[0], i[1] + len(entity1)) for i in train_pair]
    valid_pair = load_alignment_pair(lang + 'hard_split/valid_entity_seeds.txt')
    valid_pair = [(i[0], i[1] + len(entity1)) for i in valid_pair]
    test_pair = load_alignment_pair(lang + 'hard_split/test_entity_seeds.txt')
    test_pair = [(i[0], i[1] + len(entity1)) for i in test_pair]
    dev_pair = valid_pair + test_pair
    adj_matrix,r_index,r_val,adj_features,rel_features = get_matrix(triples1+triples2,entity1.union(entity2),rel1.union(rel2))
