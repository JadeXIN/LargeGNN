...
class EAData:
    def __init__(self, triple1_path, triple2_path, ent_links_path,
                 shuffle_pairs=False, train_ratio=0.3, unsup=False, **kwargs):
...
class LargeScaleEAData(EAData):
    ...
    def generate_relation_triple_dict(self):  # collect triplet, given h, its r and t; given t, its h and r.
        self.rt_dict1, self.hr_dict1 = dict(), dict()
        for h, r, t in self.triple1:
            rt_set = self.rt_dict1.get(h, set())
            rt_set.add((r, t))
            self.rt_dict1[h] = rt_set
            hr_set = self.hr_dict1.get(t, set())
            hr_set.add((h, r))
            self.hr_dict1[t] = hr_set
        print("Number of rt_dict:", len(self.rt_dict1))
        print("Number of hr_dict:", len(self.hr_dict1))
        self.rt_dict2, self.hr_dict2 = dict(), dict()
        for h, r, t in self.triple2:
            rt_set = self.rt_dict2.get(h, set())
            rt_set.add((r, t))
            self.rt_dict2[h] = rt_set
            hr_set = self.hr_dict2.get(t, set())
            hr_set.add((h, r))
            self.hr_dict2[t] = hr_set
        print("Number of rt_dict:", len(self.rt_dict2))
        print("Number of hr_dict:", len(self.hr_dict2))
